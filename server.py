...
# ---------------------------------------------------------------------------
# GraphQL helper — used by the product option linking tool
# ---------------------------------------------------------------------------
async def _graphql(query: str, variables: Optional[dict] = None, _retried: bool = False) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code == 401 and not _retried and token_manager._use_client_credentials:
            logger.warning("Got 401 on GraphQL — refreshing token and retrying...")
            await token_manager.force_refresh()
...
async def shopify_create_webhook(params: CreateWebhookInput) -> str:
    ...
    try:
        ...
        return _error(e)
# ═══════════════════════════════════════════════════════════════════════════
# PRODUCT OPTIONS (GraphQL) — link options to metaobjects for native swatches
# ═══════════════════════════════════════════════════════════════════════════
class LinkOptionToMetaobjectInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    product_id:  int = Field(..., description="Numeric Shopify product ID")
    option_name: str = Field(..., min_length=1, description="Option name to link, e.g. 'Marco' or 'Material'")
    namespace:   str = Field(default="shopify", description="Metafield namespace")
    key:         str = Field(default="color-pattern", description="Metafield key")


@mcp.tool(
    name="shopify_link_option_to_metaobject",
    annotations={"readOnlyHint": False, "destructiveHint": False, "idempotentHint": True, "openWorldHint": True},
)
async def shopify_link_option_to_metaobject(params: LinkOptionToMetaobjectInput) -> str:
    ...
    try:
        ...
        """
        result  = await _graphql(fetch_query, {"id": product_gid})
        product = result.get("product")
        if not product:
            return _error(RuntimeError(f"Product {params.product_id} not found"))
        ...
        if not target:
            available = [o["name"] for o in product.get("options", [])]
        ...
        payload = result.get("productOptionUpdate", {})
        ...
        return _fmt({
            "success":       True,
            "product_id":    params.product_id,
            "option_linked": params.option_name,
